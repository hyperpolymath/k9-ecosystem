...
and runtime contracts. `+k9-haskell+` provides a purely functional
parser and renderer for both K9 surface formats:
...
`+Pedigree+`, `+SecurityLevel+`, `+SecurityPolicy+`, `+Target+`,
`+Recipes+`, `+Validation+`, `+Contract+`, `+ContractClause+`.
...
|`+Data.K9.Parser+` |`+parseK9+`, `+parseK9File+`, `+parseK9Nickel+`,
`+detectFormat+`
...
* https://github.com/hyperpolymath/pandoc-k9[pandoc-k9] — Pandoc Lua
...
MPL-2.0 (MPL-2.0 preferred; MPL-2.0 required for Hackage). See LICENSE.
