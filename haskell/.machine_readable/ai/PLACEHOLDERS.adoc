...
sed -i 's/K9 Ecosystem Coordination Hub/my-project/g' $(grep -rl 'K9 Ecosystem Coordination Hub' .)
sed -i 's/K9_ECOSYSTEM/MY_PROJECT/g' $(grep -rl 'K9_ECOSYSTEM' .)
sed -i 's/{{project}}/my_project/g' $(grep -rl '{{project}}' .)
sed -i 's/k9-ecosystem/my-project/g' $(grep -rl 'k9-ecosystem' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `K9 Ecosystem Coordination Hub` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `{{PROJECT_DESCRIPTION}}` | One-line description | `A tool for X` | flake.nix |
| `K9_ECOSYSTEM` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `k9-ecosystem` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
